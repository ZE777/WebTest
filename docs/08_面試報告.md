...
# 附錄 C:「未來要擴充你會怎麼處理」答題框架(備查)

> 這是面試最常見的開放追問。**重點不是背一串功能,而是展現「先分維度、再沿接縫」的思考**。下面是開場句 → 五維度對照 → 三條鐵則 → 收尾句,照這個骨架講即可。

## C-1 開場句(被問「未來怎麼擴充」的第一句)

> 「我會先反問**擴充哪個維度**——是設備數量變多、資料來源變多、資料量/歷史變大、上線人數變多,還是功能變多?因為這幾種壓力點完全不同。而我在設計時**已經為主要幾種各留了一個接縫**,所以多數擴充不是重寫,是**沿接縫換實作**。」

這句贏在三點:① 不急著答、先澄清(senior 訊號);② 點出擴充是多維的;③ 強調接縫已留好 = 直接對應「架構/可維護性」評分。

## C-2 五個擴充維度 × 已留接縫 × 下一步

| 維度 | 現況 | 已留的接縫(程式真的有) | 擴充時怎麼做 | 關鍵取捨 |
|---|---|---|---|---|
| ① **設備數量**(10~20 → 數百/千) | 前端對全量做篩選/排序/分頁;KPI/排行前端 rollup | `dashboard.js` 的 **`listQuery(...)` 單一接縫**;「全量→listQuery→只渲當前頁;KPI/排行另對全量算」已在程式裡(L264) | 接縫翻後端:`GET /api/machines?page&filter&sort` + 新增 `GET /api/machines/stats`(全廠彙總);SignalR 改推「彙總快照 + 當前頁失效訊號」 | **分頁不能弄壞全廠彙總**——清單檢視/全廠彙總/即時 delta 是三種東西,彙總來源要跟「當前頁」解耦 |
| ② **資料來源**(JSON→SQL→IoT) | `IMachineDataSource` 雙實作,`DataSource:Type` 一行切換 | **介面本身就是切換縫**(Service/Controller/Hub/前端都只認介面) | 多寫一個 `class MqttDataSource : IMachineDataSource` + 一個 DI 註冊分支,上層一個字不動 | 即時來源(MQTT/Kafka)要把「拉」改「訂閱推」,`FileWatcherService` 換成 broker 訂閱;但對 Service 仍是同一個介面 |
| ③ **資料量/歷史**(40 筆 → 連續高頻) | 每台 40 筆、整段讀;趨勢回歸在**後端**算 | DTO 邊界 + 趨勢計算已在後端(前端只呈現) | 切 SQL 路徑:靠 `(MachineId, MeasuredAt DESC)` 索引取最近 N 筆;趨勢**降採樣也在後端做**;海量歷史進 time-series DB(InfluxDB)或冷熱分層 | JSON 不適合大量歷史(整份讀)——這正是「為什麼一開始就留 SQL 實作」的正當理由 |
| ④ **即時管線/上線人數**(單機 → 多節點) | 單一 IIS、單一 SignalR Hub、告警存前端 `AppState` 記憶體 | SignalR 抽象 + 只推 delta 的設計 | 多實例加 **Redis backplane**(`AddStackExchangeRedis`)讓跨節點廣播一致,前端無感 | 告警目前**純前端衍生、不落地**;要多人/多裝置一致,就得把告警上移到後端告警表 + 狀態機 |
| ⑤ **功能**(通知/權限/多廠區) | 看板唯讀、無登入、告警只活在瀏覽器 | 分層 + DTO 邊界 + `ExceptionMiddleware`;`Type` 欄位**貫穿四層**的已驗證範本 | 告警→加告警表(確認/已讀/靜音/升級)+ 通知服務(Email/SMS/Webhook);權限→ASP.NET Core Identity + 授權策略;多廠→`Machine` 加 `SiteId` 貫四層 | 每個都**沿現有分層加、不動既有**;加 `SiteId` 走的就是當初加 `Type` 完全一樣的路徑 |

## C-3 三條「擴充鐵則」(展現你想過陷阱)

1. **分頁不能弄壞全廠彙總**:儀表板不是資料表格。真要「瀏覽全部 + 分頁」的是管理清單頁 `/admin`;KPI/健康率/Top5 永遠取自**全廠全量**,不跟著當前頁走。(現在程式就是這樣:KPI 對 `AppState` 全量算)
2. **即時管線水平擴展要 backplane**:多個 IIS 節點各有自己的 SignalR Hub,A 節點收到檔案變更推播、連在 B 節點的使用者收不到 → 要 **Redis backplane** 當跨節點訊息匯流。這是 SignalR 擴展最容易漏的點。
3. **加維度沿用已驗證範本**:`Type`(設備類型)當初就是「JSON `type` → `Machine.Type` → DTO → 前端卡片」**貫穿四層**加上去的(SQL 路徑用 `AddMachineType` migration 補欄位)。未來加 `SiteId`(廠區)、`Line`(產線)走完全一樣的路徑——證明分層讓「加維度」是**線性成本,不是重構**。

## C-4 收尾句

> 「擴充對這個專案多半是『**沿接縫換實作**』而不是重寫——資料來源換實作(介面)、清單翻後端(`listQuery` 接縫)、加維度貫四層(`Type` 範本)、多節點加 backplane。我留接縫的原則是:**只在需求真的會變的地方留,不為抽象而抽象**。」

## C-5 檢查清單(自我驗收)

- [ ] 被問擴充時,第一句先**反問維度**而不是直接答
- [ ] 每個維度都能說出「**已留的接縫**」+「**下一步換什麼**」
- [ ] 講得出「分頁不能弄壞全廠彙總」這個非顯而易見的陷阱
- [ ] 知道多節點 SignalR 要 Redis backplane
- [ ] 能用 `Type` 貫四層當「加維度」的具體範本
- [ ] 結尾回到「只在會變的地方留接縫,不過度抽象」
